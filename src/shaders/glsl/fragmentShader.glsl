...
float noiseLayer(int oct, float lac, float per, float x, float y) {
    ...
    for (int o = 0; o < oct; o++) {
        n += noise(x*frequency+noise((x+1)*frequency, (y+1)*frequency, seed), y*frequency+noise((x+1)*frequency, (y+1)*frequency, seed), seed)*amplitude;
        frequency *= lac;
        amplitude *= per;
    }
    return (n+.4);
...
void main() {
    ...
    float multiplier = noiseLayer(3, 2, 0.5, vertexPosition.x+1, vertexPosition.y+1);
    vec3 color = vec3(1.0, 1.0, 1.0)*multiplier;
